...
plot(sigma[delta == 0], sigma.hat[delta == 0],
     main="null genes",
     xlab=expression(sigma[i]), ylab=expression(hat(sigma[i])),
     col=ifelse(abs.t[delta == 0] > t.tilde, "red", transgrey))
legend("topright",c(expression(abs(t[i])<tilde(t)),expression(abs(t[i])>tilde(t))),pch=1,
